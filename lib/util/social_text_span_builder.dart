...
class SocialTextSpanBuilder {
  ...
  TextSpan build(String text, {List<String>? ignoreCases, List<String>? includeOnlyCases}) {
    ...
    for (int i = 0; i < orderedMatches.length; i++) {
      ...
      root = getTextSpan(root, text.substring(match.start, match.end), secondSearch.textStyle,
          tapRecognizer: tapRecognizer2);
      cursorPosition = match.end;
    }
    if (cursorPosition < text.length - 1) {
      root = getTextSpan(
          root, text.substring(cursorPosition), getTextStyleForRange(cursorPosition, text.length).textStyle);
    }
    return root;
  }
